...
CREATE TABLE `QuizWebsiteDatabase`.`QUIZ_NAMES` (
...
CREATE TABLE `QuizWebsiteDatabase`.`QUESTIONS` (
    ...
    CONSTRAINT `fk_QUESTIONS_TO_QUIZZES` FOREIGN KEY (`QUIZ_ID`)
        REFERENCES `QuizWebsiteDatabase`.`QUIZ_NAMES` (`ID`)
...
/**
	მომხმარებლის მიერ შექმნილი ქვიზების ცხრილი
*/

CREATE TABLE `QuizWebsiteDatabase`.`CREATED_QUIZES` (
	`ID` INT NOT NULL AUTO_INCREMENT,
	`USER_ID` INT NOT NULL,
    `QUIZ_ID` INT NOT NULL,
    PRIMARY KEY (`ID`),
     INDEX `fk_CREATED_QUIZES_1_idx` (`USER_ID` ASC),
     INDEX `fk_CREATED_QUIZES_2_idx` (`QUIZ_ID` ASC),
    CONSTRAINT `fk_CREATED_QUIZES_TO_USERS` FOREIGN KEY (`USER_ID`)
        REFERENCES `QuizWebsiteDatabase`.`USERS` (`ID`),
    CONSTRAINT `fk_CREATED_QUIZES_TO_QUIZES` FOREIGN KEY (`QUIZ_ID`)
        REFERENCES `QuizWebsiteDatabase`.`QUIZ_NAMES` (`ID`)
        ON DELETE NO ACTION ON UPDATE NO ACTION
);
