...
<body>
	...
	<div id="header"></div>
	<div>
		<h1 id="h1"><%=request.getAttribute("quizName")%>
		</h1>
		<h1 id="h1"><%=request.getAttribute("description")%>
		</h1>
		<div align="center">
			<form action="takeQuiz.jsp">

				<input type="hidden" name="quizName"
					value=<%=request.getAttribute("quizName")%>>

				<%
					Quiz q = quizManager.getQuiz((String) request.getAttribute("quizName"));
					if (q.canPracticeMode()) {
						out.print(
								"Practice Mode: <input type = 'radio' name = 'practiceMode' value = 'off' checked = 'checked'> Off <input type = 'radio' name = 'practiceMode' value = 'on'> On <br>");
					} else {
				...
				Display settings: <input type="radio" name="pageDisplay"
					value="onePage" checked="checked"> One Page <input
					type="radio" name="pageDisplay" value="multiplePage">
				Multiple Page <br> <br>
				<a><input id="btn" type="submit" value="Start Quiz"></a><br>

			</form>
			<br> <br>
			<form action="NotificationServlet">
				<input type="hidden" name="sender" value="<%=username%>" /> <input
					type="text" name="receiver" /> <input type="hidden" name="type"
					value="1" /> <input type="hidden" name="message"
					value="<%=request.getAttribute("quizName")%>" /> <a><input
					type="submit" id="btn" value="Send Challenge" /> </a>
			</form>
		</div>
		<
	</div>
